import 'package:flutter/material.dart';

class AppColors {
  static const Color primaryColor = Color(0xFFFCF6ED);
  static const Color secondaryColor = Color(0xFFFF5F54);
  static const Color blue = Colors.blueAccent;
  static const Color thirdColor = Color(0xFFBBE0C4);
  static const Color fourthColor = Color(0xFFCEECFE);
  static const Color fiveColor = Color(0xFFFEF6F4);
  static const Color Black = Color(0xFF000000);
  static const Color Black15 = Color(0xFF021E40);
  static const Color BackgroundColor = Color(0xFFF3F4F9);
  static const Color NavColor = Color(0xFFE0DEF6);
  static const Color GrayLightColor = Color(0xFFFAFAFA);
  static const Color White = Color(0xFFFFFFFF);
  static const Color GreenLightColor = Color(0xFFBBE0C4);
  static const Color GREY = Color(0xffECECEC);
  static const Color grey = Colors.black26;
  static const Color LIGHT_BLACK = Color(0xff3E3E3E);
  static const Color LIGHT_Green = Color(0xff92E3A9);
  static const Color LIGHT_Red = Color(0xffef9a9a);
  static const Color DARK_ORANGE = Color(0xffef6c00);
  static const Color purple = Color(0xffB090E7);
  static const Color darkPurple = Color(0xffB090E7);
  static const Color fadedPurple = Color(0xffE0DEF6);

  static const Color yellow = Color(0xffFFCA5D);

  static const Color darkYellow = Color(0xffFFC20E);

  static const Color cardColor = Color(0xffF3F4F9);
  static const Color cardYellow = Color(0xffF8ECE2);
  static const Color cardBlue = Color(0xffE5F0F8);

  static const Color scaffoldBackground = Color(0xffF5F5F5);
  static const Color kBlue = Color(0xff8E97FD);
  static const Color kDarkBlue = Color(0xff7583CA);
  static const Color kGrey = Color(0xffA1A4B2);
  static const Color kSomo = Color(0xffFAF8F5);
  static const Color kLightGrey = Color(0xffF2F3F7);
  static const Color kLightYellow = Color(0xffFFECCC);
  static const Color kDark = Color(0xff3F414E);
  static const Color kBorderColor = Color(0xffA1A4B2);
  static const Color kSoDarkBlue = Color(0xff03174c);
  static const Color PRIMARY = Color(0xFF6479F6);
  static const Color RED = Color(0xFFF6765A);
  static const Color ACCENT = Color(0xFFFDD26C);
  static const Color CARD_COLOR = Color(0xFFF6F6F6);
  static const Color CARD_COLOR2 = Color(0xFFEFF2FE);
  static const Color WHITE = Colors.white;

  static Color DARK_GREY = Colors.grey.shade300;
  static Color DARK_GREY2 = Colors.black87;
}

...
import 'package:foxlearn/common/widgets/simple_button.dart';
...
TextStyle _kDefaultPickerTextStyle = TextStyle(letterSpacing: -0.83, fontSize: 12.0.sp);

TextStyle _themeTextStyle(BuildContext context) {
  return TextStyle(fontSize: 15.0.sp);
}
...
class _CupertinoDatePickerDateTimeState extends State<FlutterRoundedCupertinoDatePickerWidget> {
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        return itemPositioningBuilder(
          ...
          Text(
            dateText,
            style: _themeTextStyle(context)
                .copyWith(fontFamily: widget.fontFamily, color: widget.textColor),
          ),
        );
      },
    );
  ...
  Widget _buildMinutePicker(double offAxisFraction, TransitionBuilder itemPositioningBuilder) {
    return CupertinoPicker(
      scrollController:
          FixedExtentScrollController(initialItem: selectedMinute ~/ widget.minuteInterval),
      offAxisFraction: offAxisFraction,
      itemExtent: _kItemExtent,
      useMagnifier: _kUseMagnifier,
      magnification: _kMagnification,
...
class _CupertinoTimerPickerState extends State<FlutterRoundedCupertinoDurationPickerWidget> {
  ...
  Widget _buildLabel(String text) {
    return Text(
      ...
      textScaleFactor: 0.9,
      style: TextStyle(
          fontWeight: FontWeight.w600, fontFamily: widget.fontFamily, color: widget.textColor),
    );
  }
      ...
      onSelectedItemChanged: (int index) {
        setState(() {
          ...
          widget.onTimerDurationChanged(Duration(
              hours: selectedHour!, minutes: selectedMinute, seconds: selectedSecond ?? 0));
        });
      },
      children: List<Widget>.generate(24, (int index) {
    ...
    return CupertinoPicker(
      ...
      onSelectedItemChanged: (int index) {
        setState(() {
          selectedMinute = index * widget.minuteInterval;
          widget.onTimerDurationChanged(Duration(
              hours: selectedHour ?? 0, minutes: selectedMinute, seconds: selectedSecond ?? 0));
        });
      },
      children: List<Widget>.generate(60 ~/ widget.minuteInterval, (int index) {
        final int minute = index * widget.minuteInterval;
      ...
      minuteLabel = IgnorePointer(
        child: Container(
          ...
          child: Container(
            alignment: alignCenterLeft,
            width:
                widget.mode == CupertinoTimerPickerMode.ms ? _kPickerWidth / 4 : _kPickerWidth / 6,
            // Adds some spaces between words.
            padding: const EdgeInsets.symmetric(horizontal: 2.0),
            child: _buildLabel(localizations.timerPickerMinuteLabel(selectedMinute)!),
          ),
    ...
    return CupertinoPicker(
      ...
      onSelectedItemChanged: (int index) {
        setState(() {
          ...
          widget.onTimerDurationChanged(Duration(
              hours: selectedHour ?? 0, minutes: selectedMinute, seconds: selectedSecond!));
        });
      },
